...
/**
 * GOAL BINGO BACKEND (v2.0 - with Merge Logic)
 * 
 * Update Instructions:
...
function doPost(e) {
  ...
  // This ensures serial execution of merges.
    ...
    if (action === 'save') {
      ...
      if (currentDataStr) {
        try {
          ...
          // but we preserve ID/Grid mapping if Phase is Active to prevent reset.
          if (currentState.phase === 'active' && incomingState.phase === 'active') {
          ...
          incomingState.users.forEach(function(u) {
            if (!userMap[u.id]) {
               ...
               // Optimization: Could check timestamps if we added them to Users
          ...
          // Use 'lastUpdated' timestamp to determine winner per goal
          var currentGoalMap = {};
          ...
          incomingState.goals.forEach(function(incGoal) {
             ...
             if (idx !== undefined) {
                ...
                // If incoming goal is newer, update it. 
                // Using (>=) allows same-time updates to persist if they come later in the batch
