...
const GoalModal: React.FC<GoalModalProps> = ({ goal, isSetupPhase, onClose }) => {
  ...
  const userMap = React.useMemo(
    () => new Map(gameState.users.map((u) => [u.id, u])),
    [gameState.users]
  );
  ...
  const incrementHabit = (amount: number) => {
    ...
    // Add Log Locally
    addLocalLog(
      `${amount > 0 ? '完成' : '修正'} ${Math.abs(
        amount
      )} ${unit} (累計: ${newCount}/${targetCount})`
    );
  };
    ...
    <BaseModal
      ...
      title={
        <div className="flex justify-between items-center w-full gap-3">
          <div className="flex-1 min-w-0">
            <span
              className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md ${userTheme.badge} mb-1 inline-block`}
            >
              {owner.name}
            </span>
            ...
            {showSetupForm ? (
              ...
              <div className="mt-1">
                <h2 className="text-xl font-bold text-accent dark:text-white leading-tight">
                  {goal.title}
                </h2>
                <div className="flex items-center gap-2 mt-2">
                  <span className="text-[10px] px-2 py-1 rounded-md bg-brand-petrol dark:bg-brand-mint text-foreground dark:text-brand-dark font-bold flex items-center gap-1">
                    <span className="material-symbols-outlined text-[10px]">
                      {goal.structure?.type === 'habit' ? 'calendar_today' : 'flag'}
                    </span>
      ...
      footer={
        <div className="flex flex-col w-full gap-3">
          {validationError && (
            <div className="bg-white dark:bg-brand-rust/10 border border-brand-rust dark:border-brand-rust/30 text-accent text-xs font-bold p-2.5 rounded-lg flex items-center gap-2 animate-fade-in shadow-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              {!isSetupPhase && canEdit && (
                <button
                  ...
                  className={`whitespace-nowrap px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors ${
                    isEditingSettings
                      ? 'bg-brand-rust/10 text-accent border border-brand-rust/20'
                      : 'bg-brand-mint/10 text-accent border border-accent/20'
                  }`}
                >
                  <span className="material-symbols-outlined text-[16px]">
                    {isEditingSettings ? 'close' : 'edit'}
                  </span>
    ...
    >
      ...
      {showSetupForm && (
        <div className="space-y-6">
          ...
          {isEditingSettings && (
            <div className="p-3 bg-brand-rust/10 border border-brand-rust/20 rounded-xl text-accent text-xs font-bold flex gap-2">
          ...
          {canEdit && (
            <div>
              <label className="block text-xs font-black text-accent uppercase tracking-wider mb-2">
                目標類型
              </label>
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => setGoalType('habit')}
                  className={`p-4 rounded-xl border-2 text-center transition-colors flex flex-col items-center gap-2 ${
                    goalType === 'habit'
                      ? 'border-brand-teal bg-brand-teal/10 text-accent'
                      : 'border-accent/20 bg-white/50 dark:bg-black/20 text-accent'
                  }`}
                >
                  <span className="material-symbols-outlined text-[24px] mb-1 align-middle inline-block">
                    calendar_today
                  </span>
                ...
                <button
                  ...
                  className={`p-4 rounded-xl border-2 text-center transition-colors flex flex-col items-center gap-2 ${
                    goalType === 'step'
                      ? 'border-brand-rust bg-brand-rust/10 text-accent'
                      : 'border-accent/20 bg-white/50 dark:bg-black/20 text-accent'
                  }`}
                >
                  <span className="material-symbols-outlined text-[24px] mb-1 align-middle inline-block">
          ...
          {goalType === 'habit' && (
            <div className="bg-brand-mint/10 dark:bg-black/20 p-5 rounded-xl border border-brand-mint/20 dark:border-brand-teal/10 space-y-4">
              ...
              <div className="grid grid-cols-[minmax(90px,1.5fr)_auto_1fr_1fr] gap-3 items-end">
                ...
                <div className="pb-3 text-sm text-accent dark:text-accent font-bold">至少</div>
                <div>
                  <Input
                    type="number"
                    min="1"
              ...
              {frequency !== 'yearly' && (
                <div className="flex items-center gap-3">
                  <label className="text-xs font-bold text-accent whitespace-nowrap">
                    允許請假 (Buffer):
                  </label>
                  <div className="w-20">
                    <Input
                      ...
                      onChange={(e) =>
                        setExemptionCount(Math.max(0, parseInt(e.target.value) || 0))
                      }
                      className="text-center text-accent"
                    />
                  </div>
                  <span className="text-sm text-accent/60">{getFrequencyLabel()}不計分</span>
                </div>
              )}
              ...
              <div className="bg-white/50 dark:bg-black/20 p-3 rounded-xl flex justify-between items-center border border-white/20 dark:border-white/5">
                <div>
                  ...
                  <div className="text-lg font-black text-accent dark:text-accent">
                    {targetCount} {unit}
                  </div>
                </div>
                <div className="text-right text-[10px] text-accent/60">
          ...
          {goalType === 'step' && (
            <div className="bg-brand-mint/10 dark:bg-black/20 p-5 rounded-xl border border-brand-mint/20 space-y-3">
              <div className="flex justify-between items-center mb-1">
                <label className="text-xs font-bold text-accent">里程碑規劃</label>
                <span
                  className={`text-xs font-bold ${
                    totalMilestonePoints === 100 ? 'text-accent' : 'text-accent'
                  }`}
                >
                  ...
                  <div key={idx} className="flex items-center gap-2">
                    ...
                    <div className="w-20">
                      <Input
                        ...
                        value={m.points}
                        onChange={(e) =>
                          updateMilestone(idx, 'points', parseInt(e.target.value) || 0)
                        }
                        placeholder="分數"
                        className="text-center"
                      />
                    ...
                    {canEdit && (
                      <button
                        ...
                        className="text-accent/50 hover:text-accent/70 transition-colors px-1"
                      >
      ...
      {!showSetupForm && (
        <div className="space-y-6">
          {/* Score Display */}
          <div className="flex items-center justify-between bg-brand-petrol dark:bg-brand-mint text-white dark:text-brand-dark p-6 rounded-3xl shadow relative overflow-hidden">
            <div className="relative z-10">
              <div className="text-[10px] opacity-70 uppercase tracking-widest font-bold mb-1">
                Current Score
              </div>
              <div className="text-4xl font-black font-mono tracking-tight">
                {currentTotalScore}
                <span className="text-lg opacity-50 font-medium">/100</span>
              </div>
            </div>
            <div className="text-right relative z-10">
              ...
              {currentTotalScore >= 100 && canEdit ? (
                <div className="flex items-center gap-1">
                  <span className="material-symbols-outlined text-[16px] text-accent">add</span>
                  <input
                    type="number"
                    value={bonusPoints}
                ...
                <div
                  className={`font-mono font-bold text-2xl ${
                    bonusPoints > 0 ? 'text-accent' : 'text-accent/50'
                  }`}
                >
                  +{bonusPoints}
                </div>
            ...
            {goalType === 'habit' && (
              <div className="bg-brand-mint/10 dark:bg-black/20 p-6 rounded-3xl border border-brand-mint/20 text-center">
                <div className="flex items-center justify-center gap-6 mb-6">
                  {canEdit && (
                    <button
                      ...
                      className="w-12 h-12 rounded-full bg-white dark:bg-[rgb(var(--brand-dark))] border-2 border-brand-teal/20 text-accent transition-all flex items-center justify-center flex-shrink-0"
                    >
                      <svg
                        width="20"
                        height="20"
                    ...
                    {canEdit ? (
                      <div className="relative group">
                        <input
                          ...
                          onChange={(e) =>
                            setCurrentCount(Math.max(0, parseInt(e.target.value) || 0))
                          }
                          className="text-5xl font-black text-accent dark:text-white font-mono tracking-tighter bg-transparent text-center w-32 outline-none border-b-2 border-transparent focus:border-brand-teal/50 transition-colors"
                        />
                      ...
                      <div className="text-5xl font-black text-accent dark:text-white font-mono tracking-tighter">
                        {currentCount}
                      </div>
                    )}
                  ...
                  {canEdit && (
                    <button
                      onClick={() => incrementHabit(1)}
                      className="w-20 h-20 rounded-full bg-brand-petrol dark:bg-brand-mint text-foreground dark:text-brand-dark shadow transition-colors flex items-center justify-center flex-shrink-0 transform-gpu hover:bg-brand-petrol/90 dark:hover:bg-brand-mint/80"
                    >
                      <svg
                        width="32"
                        height="32"
                ...
                <div className="w-full bg-white dark:bg-[rgb(var(--brand-dark))] h-3 rounded-full overflow-hidden shadow-inner">
                  <div
                    className="bg-brand-teal h-full transition-none"
                    style={{
            ...
            {goalType === 'step' && (
              <div className="space-y-3">
                {milestones.map((m) => (
                  <label
                    key={m.id}
                    className={`flex items-center p-4 rounded-2xl border-2 transition-colors ${
                      m.isCompleted
                        ? 'bg-brand-mint/20 border-brand-mint dark:bg-brand-mint/10'
                        : 'bg-white dark:bg-[rgb(var(--brand-surface))] border-gray-100 dark:border-brand-dark'
                    } ${canEdit ? 'cursor-pointer' : 'cursor-default'}`}
                  >
                    <div
                      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-3 transition-colors flex-shrink-0 ${
                        m.isCompleted
                          ? 'bg-brand-mint border-brand-mint text-accent'
                          : 'border-gray-300 dark:border-brand-teal/30'
                      }`}
                    >
                      {m.isCompleted && (
                        <svg
                          ...
                          aria-hidden="true"
                        >
                          <path d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20zm-1 15l-4-4 1.41-1.41L11 14.17l5.59-5.59L18 10l-7 7z" />
                        </svg>
                      )}
                    </div>
                    <input
                    ...
                    <div className="flex-1">
                      <div
                        className={`font-bold ${
                          m.isCompleted ? 'text-accent dark:text-accent' : 'text-accent/60'
                        }`}
                      >
                        {m.title}
                      </div>
                    </div>
                    <div className="font-mono font-bold text-accent/60 bg-white/50 dark:bg-black/20 px-2 py-1 rounded-lg text-xs border border-accent/10">
                      {m.points}pt
                    </div>
                  </label>
                ))}
          ...
          <div className="space-y-4">
            <div className="p-4 bg-brand-purple/5 dark:bg-brand-purple/10 rounded-2xl border border-brand-purple/10 dark:border-brand-purple/20">
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-sm font-bold text-accent dark:text-purple-200 flex items-center gap-1">
                  <svg
                    width="14"
                    height="14"
                    viewBox="0 0 24 24"
                    ...
                    aria-hidden="true"
                  >
                    <path d="M12 3l2.25 4.5L19 9l-4.5 2.25L12 16l-2.25-4.5L5 9l4.75-1.5L12 3z" />
                  </svg>{' '}
                  AI 教練
                </h3>
                <button
                  onClick={fetchAdvice}
                  disabled={loadingAi}
                  className="text-xs bg-white dark:bg-brand-purple/20 text-accent dark:text-purple-100 px-3 py-1.5 rounded-full shadow-sm"
                >
                  {loadingAi ? '思考中...' : '給點建議'}
                </button>
              </div>
              {aiAdvice && (
                <p className="text-sm text-accent/90 dark:text-purple-200 italic leading-relaxed">
                  "{aiAdvice}"
                </p>
              )}
            </div>
            ...
            <div>
              {canEdit && (
                <div className="flex gap-2 mb-3">
                  <Input
                    ...
                    placeholder="寫下備註..."
                  />
                  <Button onClick={handleAddManualLog} className="whitespace-nowrap">
                    紀錄
                  </Button>
                </div>
              )}
              <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                {localLogs.map((log) => (
                  <div
                    key={log.id}
                    className="text-xs bg-white/50 dark:bg-black/20 p-3 rounded-xl border border-accent/10 flex gap-3 text-accent/70"
                  >
                    <span className="font-mono text-accent/50 whitespace-nowrap">{log.date}</span>
