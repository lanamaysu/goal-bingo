        ...
        <div className="flex justify-between items-center w-full gap-3">
          <div className="flex-1 min-w-0">
            ...
            {showSetupForm ? (
              ...
              <div className="mt-1">
                <h2 className="text-xl font-bold text-brand-petrol dark:text-white leading-tight">
                  {goal.title}
                </h2>
                ...
                <button
                  ...
                  className={`whitespace-nowrap px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors ${
                    isEditingSettings
                      ...
                      : 'bg-brand-mint/20 text-brand-teal'
                  }`}
                >
    ...
    >
      ...
      {showSetupForm && (
        <div className="space-y-6">
          ...
          {canEdit && (
            <div>
              <label className="block text-xs font-black text-brand-teal uppercase tracking-wider mb-2">
                目標類型
              </label>
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`p-4 rounded-xl border-2 text-center transition-colors flex flex-col items-center gap-2 ${
                    goalType === 'habit'
                      ? 'border-brand-teal bg-brand-teal/10 text-brand-petrol dark:text-brand-mint'
                      : 'border-brand-mint/20 bg-white dark:bg-black/20 dark:text-gray-400'
                  }`}
                >
          ...
          {goalType === 'habit' && (
            <div className="bg-brand-mint/10 dark:bg-black/20 p-5 rounded-xl border border-brand-mint/20 dark:border-brand-teal/10 space-y-4">
              ...
              <div className="grid grid-cols-[minmax(90px,1.5fr)_auto_1fr_1fr] gap-3 items-end">
                ...
                <div className="pb-3 text-sm text-brand-petrol dark:text-brand-teal font-bold">
                  至少
                </div>
                <div>
                  <Input
              ...
              {frequency !== 'yearly' && (
                <div className="flex items-center gap-3">
                  <label className="text-xs font-bold text-brand-teal whitespace-nowrap">
                    允許請假 (Buffer):
                  </label>
                  <div className="w-20">
                    <Input
              ...
              <div className="bg-white/50 dark:bg-black/20 p-3 rounded-xl flex justify-between items-center border border-white/20 dark:border-white/5">
                <div>
                  <div className="text-[10px] text-brand-teal font-bold uppercase tracking-wider">
                    年度總目標
                  </div>
              ...
              <div className="flex justify-between items-center mb-1">
                ...
                <span
                  className={`text-xs font-bold ${
                    totalMilestonePoints === 100 ? 'text-brand-teal' : 'text-brand-rust'
                  }`}
                >
              ...
              {currentTotalScore >= 100 && canEdit ? (
                ...
                <div
                  className={`font-mono font-bold text-2xl ${
                    bonusPoints > 0 ? 'text-brand-mint' : 'text-gray-500'
                  }`}
                >
                  +{bonusPoints}
                </div>
                    ...
                    {canEdit ? (
                      <div className="relative group">
                        <input
                          ...
                          className="text-5xl font-black text-brand-petrol dark:text-white font-mono tracking-tighter bg-transparent text-center w-32 outline-none border-b-2 border-transparent focus:border-brand-teal/50 transition-colors"
                        />
                      ...
                      <div className="text-5xl font-black text-brand-petrol dark:text-white font-mono tracking-tighter">
                        {currentCount}
                      </div>
                    )}

                    <div className="text-xs text-brand-teal font-bold uppercase tracking-wider mt-1">
                      / {targetCount} {unit}
                    </div>
                  </div>
                  {canEdit && (
                    <button
                      onClick={() => incrementHabit(1)}
                      className="w-20 h-20 rounded-full bg-brand-petrol text-brand-mint shadow transition-colors flex items-center justify-center flex-shrink-0 transform-gpu"
                    >
                      <svg
                        width="32"
                        height="32"
                  ...
                  >
                    <div
                      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-3 transition-colors flex-shrink-0 ${
                        m.isCompleted
                          ? 'bg-brand-mint border-brand-mint text-brand-petrol'
          ...
          <div className="space-y-4">
            <div className="p-4 bg-brand-purple/5 dark:bg-brand-purple/10 rounded-2xl border border-brand-purple/10 dark:border-brand-purple/20">
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-sm font-bold text-brand-purple dark:text-purple-200 flex items-center gap-1">
                  <svg
                    width="14"
                    height="14"
                    viewBox="0 0 24 24"
              ...
              {aiAdvice && (
                <p className="text-sm text-brand-purple/90 dark:text-purple-200 italic leading-relaxed">
                  "{aiAdvice}"
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                {localLogs.map((log) => (
                  <div
                    key={log.id}
                    className="text-xs bg-gray-50 dark:bg-black/20 p-3 rounded-xl border border-gray-100 dark:border-white/5 flex gap-3 text-gray-600 dark:text-gray-300"
                  >
                    <span className="font-mono text-gray-400 opacity-70 whitespace-nowrap">
                      {log.date}
                    </span>
