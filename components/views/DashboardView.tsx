            ...
            <>
              {!isGroupSafe && (
                <div className="mb-6 bg-white dark:bg-brand-rust/10 p-5 rounded-xl border border-brand-rust/20 dark:border-brand-rust/30 text-left">
                  ...
                  <p className="text-brand-rust font-bold text-lg">
                    {gameState.config.groupPenalty}
                  </p>
                </div>
              )}
              {isGroupSafe && (
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          <h2 className="text-3xl font-black text-brand-petrol dark:text-brand-mint flex items-center gap-2">
            <span className="material-symbols-outlined text-[32px] text-brand-rust">
              local_fire_department
            </span>{' '}
            執行中
          </h2>
          {isSolo && (
            <span className="text-xs bg-brand-mint/30 text-brand-petrol px-2 py-1 rounded-full font-bold">
              個人挑戰
            </span>
          )}
        </div>
        ...
        <div className={`grid ${isSolo ? 'grid-cols-1' : 'grid-cols-2'} gap-4`}>
          {!isSolo && (
            <div>
              ...
              <span
                className={`font-bold text-lg ${activeLines.length >= (gameState.config.minLinesForSafe || 1) ? 'text-green-600 dark:text-green-400' : 'text-brand-petrol dark:text-brand-mint'}`}
              >
          ...
          <div>
            <span className="block text-xs text-brand-teal/60">
              {isSolo ? '個人總分' : '團體總分'}
            </span>
