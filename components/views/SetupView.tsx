...
import { applySuggestionsToGoals } from '../../utils/goalHelpers';
...
const SetupView: React.FC<SetupViewProps> = ({ onSelectGoal }) => {
  ...
  const { gameState, currentUser, saveAndSync, updateGameStateLocal, startGame, resetGame, isValidating } = useGame();

  const [brainstormUser, setBrainstormUser] = useState<User | null>(null);
  const [brainstormPenalty, setBrainstormPenalty] = useState<{type: 'individual' | 'group', user?: User} | null>(null);
  ...
  const handleApplyPenalty = (penalty: string) => {
    if (brainstormPenalty?.type === 'group') {
        updateGameStateLocal({ ...gameState, config: { ...gameState.config, groupPenalty: penalty } });
    } else if (brainstormPenalty?.user) {
        const u = brainstormPenalty.user;
        updateGameStateLocal({ ...gameState, users: gameState.users.map(us => us.id === u.id ? { ...us, individualPenalty: penalty } : us) });
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-20">
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                  <h3 className="text-xl font-bold text-brand-petrol dark:text-brand-mint flex items-center gap-2">
                      <span className="material-symbols-outlined text-[24px] text-brand-teal">person</span> 我的設定
                  </h3>
                  <button 
                    onClick={() => setBrainstormUser(currentUser)}
                    className="text-xs bg-brand-purple/15 text-brand-purple dark:text-purple-200 border border-brand-purple/30 px-3 py-1.5 rounded-full hover:bg-brand-purple/25 transition-colors font-bold flex items-center gap-1 shadow-sm"
                >
            ...
            <div className="bg-white dark:bg-brand-surface p-5 rounded-2xl border border-brand-teal/20 shadow-sm">
                <div className="flex justify-between items-center mb-3">
                      ...
                      <button onClick={() => setBrainstormPenalty({type: 'individual', user: currentUser})} className="text-xs text-brand-purple bg-brand-purple/15 px-2 py-1 rounded hover:bg-brand-purple/25 border border-brand-purple/30 text-brand-purple flex items-center gap-1 shadow-sm font-bold">
                ...
                <Input 
                    ...
                    onChange={(e) => {
                        const updatedUser = { ...currentUser, individualPenalty: e.target.value };
                        updateGameStateLocal({ ...gameState, users: gameState.users.map(u => u.id === currentUser.id ? updatedUser : u) });
                    }} 
            ...
            >
                {isValidating ? (
                    ...
                    currentUser.isReady ? (
                        <>
                            <span className="material-symbols-outlined text-[20px]">check_circle</span> 已準備完成 (取消)
                        </>
                    ) : (
                        <>
                            <span className="material-symbols-outlined text-[20px]">rocket_launch</span> 我設定好了！
                        </>
                    )
                )}
            </button>
            ...
            {isSolo && allReady && (
                <div className="pt-6 border-t border-brand-teal/10 animate-fade-in">
                    ...
                    <p className="text-center text-xs text-brand-teal mt-3">按下後將鎖定目標並進入執行期</p>
                </div>
            )}
        </div>
            ...
            <div className={`space-y-6 transition-opacity duration-300 ${allReady ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                ...
                <div className="bg-white dark:bg-brand-surface p-6 rounded-2xl shadow-sm border border-brand-teal/20 space-y-5">
                    <div>
                        <div className="flex justify-between mb-2">
                            ...
                            <button onClick={() => setBrainstormPenalty({type: 'group'})} className="text-xs text-brand-purple bg-brand-purple/15 px-2 py-1 rounded hover:bg-brand-purple/25 border border-brand-purple/30 text-brand-purple flex items-center gap-1 shadow-sm font-bold">
                                <span className="material-symbols-outlined text-[12px]">lightbulb</span> 發想
                            </button>
                        </div>
                        <Input 
                            ...
                            placeholder="輸入團體懲罰..."
                            onChange={(e) => updateGameStateLocal({ ...gameState, config: { ...gameState.config, groupPenalty: e.target.value } })} 
                            className="font-bold"
                        />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-xs font-bold text-brand-teal block mb-2">連線目標 (條)</label>
                            <Input 
                                type="number" 
                                value={gameState.config.minLinesForSafe}
                                onChange={(e) => updateGameStateLocal({ ...gameState, config: { ...gameState.config, minLinesForSafe: parseInt(e.target.value) } })} 
                        ...
                        <div>
                            ...
                            <Input 
                                ...
                                onChange={(e) => updateGameStateLocal({ ...gameState, config: { ...gameState.config, groupTargetScore: parseFloat(e.target.value) } })} 
                                className="text-center font-bold"
                            />
                        </div>
                    </div>
                ...
                {allReady && (
                    <div className="pt-6 border-t border-brand-teal/10">
                        ...
                        <p className="text-center text-xs text-brand-teal mt-3">按下後將隨機打亂九宮格並進入執行期</p>
                    </div>
                )}
            </div>
        )}
      ...
      <div className="mt-12 pt-8 border-t-2 border-dashed border-brand-teal/20">
         <div className="flex justify-between items-center bg-brand-rust/5 p-4 rounded-xl border border-brand-rust/10">
             <div>
                 <h4 className="font-bold text-brand-rust flex items-center gap-2"><span className="material-symbols-outlined text-[16px]">refresh</span> 重置隊伍</h4>
                 <p className="text-xs text-brand-rust/70 mt-1">此動作將清除所有成員與目標，讓隊伍重新加入。</p>
             </div>
             <button 
                onClick={() => setShowResetConfirm(true)}
                className="px-4 py-2 bg-white dark:bg-brand-dark border border-brand-rust/30 text-brand-rust font-bold rounded-xl hover:bg-brand-rust hover:text-white transition-colors text-sm"
             >
                 重置設定
             </button>
         </div>
      ...
      <ConfirmDialog 
        ...
        title={
            ...
            ? <span className="flex items-center justify-center gap-2 text-brand-rust"><span className="material-symbols-outlined">warning</span> 強制開始遊戲</span> 
            : <span className="flex items-center justify-center gap-2 text-brand-petrol dark:text-brand-mint"><span className="material-symbols-outlined">rocket_launch</span> 開始遊戲</span>
        }
      ...
      <ConfirmDialog 
        ...
        title={<span className="flex items-center justify-center gap-2 text-brand-rust"><span className="material-symbols-outlined">warning</span> 確定重置隊伍？</span>}
        message={"這將會：\n1. 清除所有已加入的成員\n2. 清除所有已設定的目標\n3. 讓其他人必須重新加入遊戲\n\n此動作無法復原！"}
        confirmText="確定重置"
        cancelText="取消"
        isDestructive={true}
      ...
      <AlertDialog 
        ...
        title={<span className="flex items-center justify-center gap-2 text-brand-rust"><span className="material-symbols-outlined">error</span> 設定尚未完成</span>}
