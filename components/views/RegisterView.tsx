...
const RegisterView: React.FC<RegisterViewProps> = ({ 
    year, name, isSaving, errorMsg, isGameFull = false, existingUsers = [],
    onNameChange, onRegister 
}) => {
        ...
        <div className="space-y-6">
          ...
          {!isRecoveryMode && (
              <div className="animate-fade-in">
                 <label className="block text-center text-xs font-bold text-brand-teal dark:text-brand-teal/80 mb-3 uppercase tracking-wider">選擇代表色</label>
                 <div className="flex justify-center flex-wrap gap-3">
                     {USER_THEME_PALETTE.map((theme) => (
                         ...
                         >
                            {selectedColorId === theme.id && (
                                <span className="material-symbols-outlined text-[16px] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">check</span>
                            )}
                         </button>
                     ))}
                 </div>
