...
const BrainstormModal: React.FC<BrainstormModalProps> = ({ 
    isOpen, onClose, targetName, mode, maxSelectable = 3, onApplyGoal, onApplyPenalty 
}) => {
  ...
  const getTypeBadge = (type: string) => {
      switch(type) {
          case 'step': return <span className="bg-brand-rust/10 text-brand-rust dark:text-orange-200 text-[10px] px-1.5 py-0.5 rounded border border-brand-rust/20 dark:border-brand-rust/30">階段型</span>;
          case 'habit': return <span className="bg-brand-teal/10 text-brand-teal dark:text-brand-mint text-[10px] px-1.5 py-0.5 rounded border border-brand-teal/20 dark:border-brand-mint/30">規律型</span>;
          default: return null;
      }
  };
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-2">
                <Input 
                    ...
                    placeholder={mode === 'goal' ? "興趣關鍵字 (如：烹飪、馬拉松)" : "關鍵字 (如：請客、運動)"}
                    onKeyDown={(e) => {
            ...
            <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-1">
                        ...
                        >
                            <div className="flex justify-between items-start mb-1">
                                <div className="flex items-center gap-2">
                                    <div className={`w-4 h-4 rounded border flex items-center justify-center ${isSelected ? 'bg-brand-purple border-brand-purple' : 'border-brand-purple/30 bg-white dark:bg-transparent'}`}>
                                        {isSelected && <span className="text-white text-xs material-symbols-outlined text-[12px]">check</span>}
                                    </div>
                                    <span className="font-bold text-brand-petrol dark:text-brand-mint text-sm">{s.title}</span>
                                </div>
                            ...
                            <div className="text-xs text-brand-petrol/70 dark:text-brand-mint/70 pl-6 mb-1">{s.description}</div>
                ...
                {/* Penalty Suggestions */}
                {mode === 'penalty' && penaltySuggestions.map((s, idx) => (
                    <div key={idx} className="flex items-center justify-between gap-4 p-3 bg-white dark:bg-brand-rust/20 rounded-xl border border-brand-rust/10 dark:border-brand-rust/30 group">
                        <div className="text-sm text-brand-rust dark:text-orange-200 font-medium flex-1">{s}</div>
                        <Button 
                            variant="danger"
                            onClick={() => { onApplyPenalty?.(s); resetAndClose(); }}
                ...
                {(mode === 'goal' ? goalSuggestions.length === 0 : penaltySuggestions.length === 0) && !loading && !error && (
